...
# ==================== ВКЛАДКА 4: СПРАВКА ====================
with tab4:
    st.header("🆘 Справка по использованию приложения")
    
    # Создаем аккордеон для разных разделов справки
    with st.expander("пш Быстрый старт", expanded=True):
        st.markdown("""
        **Как начать работу с приложением:**
        
        1. **Запустите API сервер** (если он еще не запущен):
           ```bash
           uvicorn api:app --reload
        2. Вводите текст на любом языке и посмотрите результат
        3. Смотрите статистику данных о языках
                    """)